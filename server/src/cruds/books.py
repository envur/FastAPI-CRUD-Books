...
def get_books_by_user(db: Session, user_id: int):
    return db.query(b_models.Books).filter(b_models.Books.owner_id == user_id).all()
...
def update_book(db: Session, user_id:int, book: b_schemas.BookUpdateRequest, book_id: int):
    db_book = get_book_by_id(db, book_id)
    if db_book.owner_id == user_id:
        db_book_update = update(b_models.Books).where(b_models.Books.id == book_id).values(**book.book.dict(exclude_unset=True))
